...
https:\/\/shop40317240\.youzan\.com\/wscump\/lottery\/join-lottery url script-request-header yunmai.cookie.js

hostname = *.youzan.com, *.iyunmai.com
*/
...
if (
  ...
  $request.url.match(/join\-lottery\.json/)
) {
  const cjurl = $request.url;
  $.log(cjurl);
  const cjheaders = JSON.stringify($request.headers);
  $.log(cjheaders)
  $.setdata(cjurl, lotteryurl);
  $.setdata(cjheaders, lotterycookie);
  $.msg("云麦好轻", "", "获取抽奖Cookie成功");
}
